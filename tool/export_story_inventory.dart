...
class CreatureEntryView {
  const CreatureEntryView({
    required this.id,
    required this.name,
...
void main() {
  ...
  final docsDir = Directory('${repoRoot.path}/docs');
  if (!docsDir.existsSync()) {
    docsDir.createSync(recursive: true);
  }

  final creaturesPath = File(
    '${repoRoot.path}/assets/data/alchemons_creatures.json',
  );
  final creaturesJson =
      jsonDecode(creaturesPath.readAsStringSync()) as Map<String, dynamic>;
  final creatures = (creaturesJson['creatures'] as List<dynamic>)
      .cast<Map<String, dynamic>>()
      .map(
        (entry) => CreatureEntryView(
          id: entry['id'] as String,
          name: entry['name'] as String,
          family: entry['mutationFamily'] as String? ?? 'Unknown',
          types: ((entry['types'] as List<dynamic>?) ?? const [])
              .map((value) => value.toString())
              .toList(),
          rarity: (entry['rarity'] as String?) ?? 'Unknown',
          description: (entry['description'] as String?) ?? '',
        ),
      )
      .toList();

  final storyAudit = File('${docsDir.path}/story_mode_content_inventory.md');
  storyAudit.writeAsStringSync(_buildStoryInventoryMarkdown());

  final descriptions = File('${docsDir.path}/alchemon_descriptions.md');
  descriptions.writeAsStringSync(_buildCreatureDescriptionsMarkdown(creatures));

  stdout.writeln('Wrote ${storyAudit.path}');
  stdout.writeln('Wrote ${descriptions.path}');
}

String _buildStoryInventoryMarkdown() {
  final sections = <StorySection>[
    const StorySection(
      title: 'Boot / Replay Story Intro',
      source:
          'lib/main.dart, lib/screens/profile_screen.dart, lib/screens/story/story_intro_screen.dart, lib/screens/story/models/story_page.dart',
      reachability:
          'Reachable on first launch, after reset-to-new-game, and from Profile -> Replay Story.',
      items: [
    ...
    const StorySection(
      title: 'StoryManager: firstBreeding Queue',
      source:
          'lib/screens/story/models/story_page.dart + lib/screens/breed/breed_screen.dart',
      reachability:
          'Currently the only StoryEvent wired through StoryManager. Fires after the first extraction completes.',
      items: [
    ...
    const StorySection(
      title: 'Starter / Extraction Setup',
    ...
    const StorySection(
      title: 'Wilderness Tutorial Flow',
      source: 'lib/screens/scenes/scene_page.dart',
      reachability:
          'Reachable in tutorial wilderness mode. Presented before the first forced encounter.',
      items: [
    ...
    const StorySection(
      title: 'Post-Planet Wilderness Beat',
      source: 'lib/screens/scenes/scene_page.dart',
      reachability:
          'Reachable once after the first cosmic planet-entry story has happened, on first visit to valley/sky/swamp/volcano.',
      items: ['[Dialog] Self Deception', 'Does reality dictate beauty?'],
    ),
    const StorySection(
      title: 'Cosmic: First Planet Entry',
      source: 'lib/screens/cosmic/cosmic_screen.dart',
      reachability:
          'Reachable on the first pathway entry into a cosmic planet scene.',
      items: [
    ...
    const StorySection(
      title: 'Cosmic: Recipe / Pathway Guidance',
      source: 'lib/screens/cosmic/cosmic_screen.dart',
    ...
    const StorySection(
      title: 'Cosmic: Ship Discovery',
      source: 'lib/screens/scenes/scene_page.dart',
      reachability:
          'Reachable after visiting all four core biomes and finding the ship in valley.',
      items: [
    ...
    const StorySection(
      title: 'Cosmic: Blood Ring Ending',
    ...
    const StorySection(
      title: 'Cosmic: Blood Ring Gate',
      source: 'lib/screens/cosmic/cosmic_screen.dart',
      reachability:
          'Reachable if the player finds the Blood Ring before the first planet-entry pathway story has been seen.',
      items: [
        '[Gate Message] Complete planetary recipes to unlock a sacrifice.',
      ],
    ),
    const StorySection(
      title: 'Boss Gauntlet Intro',
      source: 'lib/screens/boss/boss_intro_screen.dart',
      reachability:
          'Reachable once on first entry to the boss gauntlet screen.',
      items: [
        '[Dialog] Alchemy is Power?',
    ...
    const StorySection(
      title: 'Blood Boss Gate Beats',
      source: 'lib/screens/boss/boss_intro_screen.dart',
      reachability:
          'Reachable around the final blood-boss unlock path. The unlock beat fires after first defeating boss 016; the lock popup appears if the constellation finale has not been seen; the victory beat fires after first defeating the blood boss.',
      items: [
    ...
    const StorySection(
      title: 'Boss Relic / Mystic Altar Intro',
      source: 'lib/screens/mystic_altar/boss_altar_detail_screen.dart',
      reachability:
          'Reachable once on first entry to the boss relic altar detail screen.',
      items: [
    ...
    const StorySection(
      title: 'Blood Mystic Altar Beats',
      source: 'lib/screens/mystic_altar/boss_altar_detail_screen.dart',
      reachability:
          'Reachable in the blood relic altar flow. The altar intro appears once on first entry to the blood mystic ritual; the witness section is a persistent requirement on the blood ritual UI; the space hint appears once after first successfully summoning the blood mystic.',
      items: [
        '[Dialog] Not A Return',
    ...
    const StorySection(
      title: 'Pure Lineage Extraction Intro',
      source: 'lib/widgets/pure_breeding_intro_dialog.dart',
      reachability:
          'Reachable once on first pure extraction result. More explanatory than dramatic, but still part of the narrative framing around breeding.',
      items: [
    ...
    const StorySection(
      title: 'Constellation Story Fragments',
      source: 'lib/games/constellations/constellation_game.dart',
      reachability:
          'Reachable as animated connection text in the constellation trees.',
      items: [
    ...
    const StorySection(
      title: 'Cosmic Contest Hint Lore',
      source: 'lib/games/cosmic/cosmic_contests.dart',
      reachability:
          'Reachable as collectible hint notes in cosmic contest arenas. Not main story, but definitely authored quote/lore content.',
      items: [
  ...
  final buffer = StringBuffer()
    ..writeln('# Story Mode Content Inventory')
    ..writeln()
    ..writeln('Generated from authored text found in the repo.')
    ..writeln()
    ..writeln('## Active Story Notes')
    ..writeln()
    ..writeln(
  ...
  for (final section in sections) {
    buffer
      ...
      ..writeln()
      ..writeln('- Source: `${section.source}`')
      ..writeln('- Reachability: ${section.reachability}')
      ..writeln();
  ...
  buffer
    ..writeln('## Suggested Optimization Targets')
    ..writeln()
    ..writeln(
...
String _buildCreatureDescriptionsMarkdown(List<CreatureEntryView> creatures) {
  const familyOrder = [
    'Let',
    'Pip',
    'Horn',
  ...
  final grouped = <String, List<CreatureEntryView>>{};
  for (final creature in creatures) {
    grouped
        .putIfAbsent(creature.family, () => <CreatureEntryView>[])
        .add(creature);
  }
  ...
  final buffer = StringBuffer()
    ..writeln('# Alchemon Descriptions')
    ..writeln()
    ..writeln(
      'Generated from `assets/data/alchemons_creatures.json`. Total creature entries: ${creatures.length}.',
    )
    ..writeln()
    ..writeln('Family totals:')
    ..writeln();

  for (final family in familyOrder) {
    final entries = grouped[family];
    if (entries == null) continue;
    buffer.writeln('- $family: ${entries.length}');
  }
  ...
  for (final family in familyOrder) {
    ...
    buffer
      ..writeln('## $family (${entries.length})')
      ..writeln();

    for (final creature in entries) {
      final typeLabel = creature.types.join('/');
      buffer.writeln(
        '- `${creature.id}` | ${creature.name} | $typeLabel | ${creature.rarity}: ${creature.description}',
      );
    }
