...
class DevSeeder {
  ...
  Future<void> createTwoTestEggs() async {
    ...
    final slots = await db.incubatorDao.watchSlots().first; // single snapshot
    final slot0 = slots.firstWhere((s) => s.id == 0);
    final slot1 = slots.firstWhere((s) => s.id == 1);

    // make sure they’re unlocked
    if (!slot0.unlocked) await db.incubatorDao.unlockSlot(0);
    if (!slot1.unlocked) await db.incubatorDao.unlockSlot(1);

    // clear any eggs that might be there already
    if (slot0.eggId != null) await db.incubatorDao.clearEgg(0);
    if (slot1.eggId != null) await db.incubatorDao.clearEgg(1);
