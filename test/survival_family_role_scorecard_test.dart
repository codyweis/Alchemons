  ...
  testWidgets('family role scorecard', (tester) async {
    ...
    final buffer = StringBuffer()
      ...
      ..writeln(
        ...
        '"Orb dmg" is damage the orb took; baseline is the ship alone. '
    ...
    for (final scenario in RoleScenario.values) {
      ...
      buffer
        ...
        ..writeln()
        ..writeln(
          '| Family | Orb dmg | Leak body-s | Target kills | '
          'Heavy HP removed | Ship dmg | Cleared at | Healing | Deaths | '
      ...
      for (final family in _kFamilies.keys) {
        ...
        double merit(RoleResult r) => switch (scenario) {
          RoleScenario.tide => -r.orbDamage - r.leakSeconds * 2,
          RoleScenario.artillery =>
            r.targetKills * 40 - r.shipDamage - r.orbDamage,
          RoleScenario.brood =>
            r.targetKills * 60 - r.orbDamage - r.leakSeconds,
          RoleScenario.siege => r.heavyShare * 4000 - r.orbDamage,
          RoleScenario.flank => -(r.orbDamage + r.shipDamage),
        };
