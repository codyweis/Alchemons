// REQUIA, IN GLASS — the states Spirit's glass is meant to show
// (docs/dungeons.md §7.11), asserted to be different pictures.
//
// The rooms whole are `planet_dungeon_whole_room_audit_test.dart
// --dart-define=AUDIT=Spirit`; this is the STATES. The kept dream window is shot from a
// game where only the maxim's id is set — a found maxim's mark must stand on
// every later descent.
//
// `mkdir -p build/room_audit` and run this to get the PNGs (SpiritGlass_*.png).
...
Future<PlanetDungeonGame> _game(String roomId, {int stars = 0}) async {
  const els = ['Spirit', 'Water', 'Crystal'];
...
void main() {
  ...
  testWidgets('every state the grave\'s glass shows', (tester) async {
    await tester.runAsync(() async {
      ...
      Future<void> shoot(
        ...
        if (out.existsSync()) {
          ...
          File(
            'build/room_audit/SpiritGlass_$name.png',
      ...
      // A barrow's sigil stone: to a warm eye, to a cold one, and taken.
      await shoot('sigil_warm', 'barrow_urn');
      await shoot(
        'sigil_cold',
        'barrow_urn',
        setup: (g) => g.wake.field.world = GraveWorld.ghost,
      );
      await shoot(
        'sigil_stamped',
        'barrow_urn',
        setup: (g) => g.wake.field.sigilStamped = true,
      );

      // The undug grave: blank, the window lighting as the rite binds, and
      // the window on a later descent.
      await shoot('undug', 'mourners_walk');
      await shoot(
        'undug_rite',
        'mourners_walk',
        setup: (g) => g.beginMaximRite(
          kSpiritStuffOfDreamsEggId,
      ...
      await shoot(
        'undug_kept',
        'mourners_walk',
        setup: (g) => g.discoveredClouds.add(kSpiritStuffOfDreamsEggId),
      );

      for (final (a, b) in const [
        ('sigil_warm', 'sigil_cold'),
        ('sigil_warm', 'sigil_stamped'),
        ('undug', 'undug_rite'),
        ('undug_rite', 'undug_kept'),
      ]) {
        expect(
          shots[a] != shots[b],
          isTrue,
