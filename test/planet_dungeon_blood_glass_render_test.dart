// HEMAVORN, IN GLASS — the states Blood's glass is meant to show
// (docs/dungeons.md §7.11), asserted to be different pictures.
//
// The rooms whole are `planet_dungeon_whole_room_audit_test.dart
// --dart-define=AUDIT=Blood`; this is the STATES. The kept hearts is shot from a
// game where only the maxim's id is set — a found maxim's mark must stand on
// every later descent.
//
// `mkdir -p build/room_audit` and run this to get the PNGs (BloodGlass_*.png).
...
Future<PlanetDungeonGame> _game(String roomId, {int stars = 0}) async {
  ...
  final b = kPlanetDungeonLayouts['Blood']!.rooms[roomId]!.bounds;
...
void main() {
  ...
  testWidgets('every state the heart\'s glass shows', (tester) async {
    await tester.runAsync(() async {
      ...
      Future<void> shoot(
        ...
        if (out.existsSync()) {
          ...
          File(
            'build/room_audit/BloodGlass_$name.png',
      ...
      // The arch's two cocks: untouched, one vessel grafted, and the garnet
      // hearts lighting as the rite binds and kept on a later descent.
      await shoot('arch', 'aortic_arch');
      await shoot(
        'arch_grafted',
        'aortic_arch',
        setup: (g) => g.heart.grafted.add('co_diagonal'),
      );
      await shoot(
        'arch_rite',
        'aortic_arch',
        setup: (g) => g.beginMaximRite(kBloodLifeEggId, const Offset(180, 350)),
        seconds: 1.0,
      );
      await shoot(
        'arch_kept',
        'aortic_arch',
        setup: (g) => g.discoveredClouds.add(kBloodLifeEggId),
      );
